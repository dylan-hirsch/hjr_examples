...
@app.cell
def _(CanoeDynamicsBall, ClosedLoopTrajectory, hj, jnp, np):
    ...
    def compute_value(vox=100, Lambda=0.5):
    ...
    def compute_value_HJR(vox=100, Lambda=0.5):
...
def _(compute_value, compute_value_HJR):
    ...
    for i in [0, 1, 2]:
        ...
        HJR_cls.append([])
        for j in [0, 1, 2]:
            V, grid, cl = compute_value(vox=voxes[i], Lambda=Lambdas[j])
            Vs[i].append(V[-1, ...])
            grids[i].append(grid)
            cls[i].append(cl)

            V, grid, cl = compute_value_HJR(vox=voxes[i], Lambda=Lambdas[j])
            HJR_Vs[i].append(V[-1, ...])
            HJR_grids[i].append(grid)
            HJR_cls[i].append(cl)
    return HJR_Vs, HJR_cls, HJR_grids, Lambdas, Vs, cls, grids, voxes
...
def _(
    ...
    def _(Vs, grids, cls, term=r"$\lambda$", level=0.0):
    ...
    fig, axs = _(Vs, grids, cls, level=0.01)
    fig.savefig(
        "/Users/dylanhirsch/Desktop/reach_avoid_sum_of_rewards.png",
        bbox_inches="tight",
    )

    fig, axs = _(HJR_Vs, HJR_grids, HJR_cls)
    fig.savefig(
